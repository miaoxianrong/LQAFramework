...
String includerID,includerName,ParentName;
...
if(bSearch){
	iOnePage=200;
	sql="select count(*) from includerList where includerID>0 and ProjectID='"+ProjectID+"' ";
...
if(bSearch){
	sql="select *  from includerList where includerID>0 ";
	if(SearchType.compareTo("RegDate")==0||SearchType.compareTo("ProjectDate")==0){
		sql+=" and "+SearchType+" <= '"+SearchContent+"'";		
	}
	else{
	...
	sql="select * from includerList where includerID>0 ";
	sql=sql+" and ProjectID='"+ProjectID+"' order by includerID asc";
}
//out.print(sql+"<BR>");
RS =workM.getSQL(sql); 
%>
...
<form method="POST" action="includer.jsp" name="Maintain" >
  <input type=hidden name="ProjectID" value="<%=ProjectID%>">
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="5" align=center><span class="zi"><b>Includer List</b></span></td>
  </tr>
  <tr bgcolor="#F8FDFF"> 
    <td height="25" colspan="5"><SELECT name=SearchType size=1 class="tb">
<OPTION value="includerName">includerName</OPTION>
...
<OPTION value="DESC">DESC</OPTION>
  ...
  <tr bgcolor="#F8FDFF"> 
    ...
    <td align="center">
    <a href="includer_add.jsp?ProjectID=<%=ProjectID%>"><IMG alt='Edit'  border=0 name=22.1.1.6.2 src="images/add.gif"></a>
    </td>
  </tr>
  <tr bgcolor="#F8FDFF"> 
...
%>
  ...
  <tr bgcolor="#F8FDFF" onmouseover="this.bgColor='#EEF2F4';" onmouseout="this.bgColor='#F8FDFF';"> 
    <td height="25"> 
      <div align="center"><INPUT name=now_dn type=checkbox value="<%=includerID%>"></div></td>
    <td height="25"> 
      <div align="center"><a href="includer_edit.jsp?ProjectID=<%=ProjectID%>&includerID=<%=includerID%>"><%=includerName%></a></div>
    </td>
    <td height="25"> 
      <div align="center"><a href="includer_edit.jsp?ProjectID=<%=ProjectID%>&includerID=<%=includerID%>"><%=ParentName%></a></div></td>
   
    <td height="25"> 
      <div align="center"><a href="includer_edit.jsp?ProjectID=<%=ProjectID%>&includerID=<%=includerID%>">
      <%
    ...
    <td height="25"> 
      <div align="center">
      <a href="includer_edit.jsp?ProjectID=<%=ProjectID%>&includerID=<%=includerID%>"><IMG alt='Edit'  border=0 name=22.1.1.6.2 src="images/cn_1_6_e.gif"></a> 
      <a href="includer_del.jsp?ProjectID=<%=ProjectID%>&includerID=<%=includerID%>&OrderValue=<%=iCurPage%>" onclick="return del()"><IMG alt='Delete'  border=0 name=22.1.1.6.2 src="images/delete.gif"></a></div></td>
  ...
  <tr bgcolor="#F8FDFF"> 
    <td height="25" colspan="5"> 
     <div align="center">
     <A href="includer.jsp?NextPage=No&ProjectID=<%=ProjectID%>" style="TEXT-DECORATION: none"><FONT color=black>
	<%if(iCurPage==1){%> First <%} else {%>Previous <%}%></FONT> </A> 
	<A href="includer.jsp?NextPage=Yes&ProjectID=<%=ProjectID%>" style="TEXT-DECORATION: none"><FONT color=black>
...
<INPUT name=name_array type=hidden> 
	...
	<TABLE class=table_popup_opt id=id_line10>
	  ...
	  <TR>
	    <TD>
	    <A href="about:blank" onclick="Maintain.action='includer_add.jsp?OrderValue=<%=iCurPage%>&ProjectID=<%=ProjectID%>';Maintain.submit();return false;">Add one xpath</A>
	    <BR>
	    <A href="about:blank" onclick="Maintain.action='includer_clear.jsp?OrderValue=<%=iCurPage%>&ProjectID=<%=ProjectID%>';Maintain.submit();return false;">Clear xpath</A>
	    <BR>
	    <A href="about:blank" onclick="nn=Judge(Maintain); if (nn>0) {if (confirm('Confirm delete?')){Maintain.action='includer_del_mul.jsp?OrderValue=<%=iCurPage%>&ProjectID=<%=ProjectID%>';OnSubmit(Maintain);Maintain.submit(); return false;} else {return false;}}  else {alert('Please select records');return false;}">Delete multi xpath</A>
	    </TD>
	  </TR>
	  <TR>
	    <TD>
