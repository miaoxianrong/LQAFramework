...
<form method="POST" action="monitor.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    ...
    <a href='monitor.jsp?ProjectID=<%=ProjectID%>'>All</a>
    <%
	...
	if(j==1){
		...
		middleNodePath=middleNodePath+"-"+getSeq;
     	...
     	%>
     	&nbsp;&nbsp;<a href='monitor.jsp?ProjectID=<%=ProjectID%>&node=<%=middleNodePath%>'><%=middleNodePath%></a>
     	<%
    }
    %>
    </b></span></td>
...
%>
  ...
  <tr bgcolor="<%=bgcolor%>"> 
    ...
    <%if(bSpecial){
    	...
    	<a href='monitor.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>&debugging=quick'><font color='<%=runcolor%>'></font></a>
    <%}%>
    </div></td>
