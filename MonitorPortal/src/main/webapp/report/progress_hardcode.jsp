...
<title>I18N Testing - Hardcode</title>
...
RootURLNodePath="4"; 
LevelID="2";
String sql_aux=" and special=1 and LevelID>0";
workM.getConnection();
...
int iPlocNum=0;
String sql_screen="select count(*) from NodeList where playonce=1 and ProjectID='"+ProjectID+"'";
ResultSet rs_screen=workM.getSQL(sql_screen);
if(rs_screen.next()){
	iPlocNum=rs_screen.getInt(1);
}
rs_screen.close();


int iTotalSpecial=0;
String sql3="select count(*) from NodeList where special=1 and ProjectID='"+ProjectID+"'";
ResultSet rs3=workM.getSQL(sql3);
if(rs3.next()){
	iTotalSpecial=rs3.getInt(1);
}
int iCoverage=0;
if(iTotalSpecial==0){
	iCoverage=1;
}else{
	iCoverage=100*iPlocNum/iTotalSpecial;
}
...
sql+=" order by playonce desc";
//sql+=" order by playonce desc"; //playonce
...
<form method="POST" action="progress_hardcode.jsp" name="Maintain" >
  <input type=hidden name='ProjectID' value='<%=ProjectID%>'>
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="16" align=left><span class="zi"><b>
    Navigation(<font color=red><B>Total:<%=iTotalSpecial%>,Coverage: <%=iCoverage%>%,Screens=<%=iPlocNum%></B></font>):&nbsp;&nbsp;&nbsp;&nbsp;
    <a href='progress_hardcode.jsp?ProjectID=<%=ProjectID%>'>All</a>
    <%
	...
	if(j==1){
		...
		middleNodePath=middleNodePath+"-"+getSeq;
     	...
     	%>
     	&nbsp;&nbsp;<a href='progress_hardcode.jsp?ProjectID=<%=ProjectID%>&node=<%=middleNodePath%>'><%=middleNodePath%></a>
     	<%
    }
    %>
    </b></span></td>
...
<%  
  ...
  while(RS.next())
  {
	  ...
	  {//
	  	...
	  	playonce=RS.getString("playonce");
	  	if(playonce.compareTo("1")==0){
	  		playonce="playonce";
	  		bgcolor="green";
	  	}
		...
		Feature="";
		iHardcode=Integer.parseInt(RS.getString("iHardcode"));
%>
  ...
  <tr bgcolor="<%=bgcolor%>"> 
    ...
    <td height="25"><div align="left">
    	<%if(iLevelID==1){%>
    	<a href='progress_hardcode.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>'><%=NodePath%></a>
    	<%}%>
    </div></td>
     <td height="25"><div align="left">
    	<%if(iLevelID==2){%>
    ...
    <%if(bSpecial){
    	...
    	<a href='progress_hardcode.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>&debugging=quick'><font color='<%=runcolor%>'></font></a>
    <%}%>
    </div></td>
    <td height="25"><div align="center">
    <%if(iHardcode==1){%>
    <a target='rightFrame' href='../screen/<%=Feature%><%=NodePath%>.png'><%=NodePath%>.png</a>
    <%}%><%=iHardcode%>
    </div></td>
