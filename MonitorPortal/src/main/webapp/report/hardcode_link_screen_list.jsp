...
<body bgcolor="#F8FDFF">
<%
String HardcodeListID,tagName,nodeId,screenName,nodeClass,nodeType,nodeTitle,childNum,nodeText,nodeHref,PageFilter;
...
String Hardcode="";
String sql_hardcode="select hardcode from HardcodeList where ProjectID='"+ProjectID+"' and UniqueStringListID="+UniqueStringListID;
ResultSet rs_hardcode=workM.getSQL(sql_hardcode);
if(rs_hardcode.next()){
	Hardcode=rs_hardcode.getString("Hardcode");
}
rs_hardcode.close();

RootURLNodePath="4"; 
LevelID="2";
String sql_aux=" and UniqueStringListID="+UniqueStringListID+" ";



int iTotalScreenNum=0;
String sql2="select count(*) from HardcodeList where ProjectID='"+ProjectID+"' and UniqueStringListID="+UniqueStringListID;
...
if(bSearch){
	iOnePage=1000;
	sql="select count(*)  from HardcodeList";
	if(SearchContent.compareTo("")==0){
		sql+=" where HardcodeListID>0 ";
	}
	else{
...
<form method="POST" action="guilist.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    ...
    Hard code string: <font color=blue><B><%=Hardcode%></B></font><BR>
    Corresponding screens: <font color=blue><B><%=iTotalScreenNum%></B></font>
    </div></td>
  </tr>  
  <tr bgcolor="#F8FDFF"> 
...
<%  
  ...
  while(RS.next())
  {
	  ...
	  {//·ÖÒ³
	  	HardcodeListID=RS.getString("HardcodeListID");
...
%>
  ...
  <tr bgcolor="<%=bgcolor%>"> 
    <td height="25"> 
      <div align="center"><INPUT name=now_dn type=checkbox value="<%=HardcodeListID%>"></div>
    </td>
     <td height="25"><div align="left">
    	<a target='rightFrame' href='hardcode_link_screen.jsp?ProjectID=<%=ProjectID%>&NodePath=<%=NodePath%>&screenName=<%=screenName%>'><%=screenName%></a>
    </div></td>
     <td height="25"><div align="left">
  ...
  %>
   <tr bgcolor="#F8FDFF"> 
    <td height="25" colspan="3"> 
      <div align="left">Total screens: <%=iSpecial%> </div></td>
  </tr>
