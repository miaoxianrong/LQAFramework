...
<title>Not Mapping Reason Analysis</title>
...
<form method="POST" action="navigation.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="4" align=center><span class="zi"><%=ProjectID%> - Not mapped reason list</span></td>
  </tr>
  <tr bgcolor="<%=bgcolor%>">     
  ...
  <tr bgcolor="#A3DBE8">     
    <td height="25">Mapped</td>
    <td height="25"><%=iMapped%></td>
  ...
  <tr bgcolor="#A3DBE8">     
    <td height="25">Not Mapped</td>
    <td height="25"><%=iNotMapped%></td>
    <td height="25"><%=(100*iNotMapped)/iAll%>%</td>
    <td height="25"></td>
  </tr>
  <tr bgcolor="<%=bgcolor%>">     
    <td height="25">Unknown reason</td>
    <td height="25">
    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=0'><font color=blue><%=iUnknown%></font></a>
    </td>
    <td height="25"><%=(100*iUnknown)/iAll%>%</td>
    <td height="25">Research</td>
  </tr>
  ...
  while(stArray.hasMoreTokens()){
  	...
  	rs.close();
  	sql="select count(*) from I18NFile where ReasonID="+ReasonID+" and ProjectID='"+ProjectID+"'"; 
  	rs=workM.getURL(sql); 
  	if(rs.next()){
  		iNumber=rs.getInt(1);
  		if(iAll>0){
	  	...
	  	<tr bgcolor="<%=bgcolor%>">     
		    ...
		    <td height="25">
		    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=<%=ReasonID%>'><font color=blue><%=iNumber%></font></a>
		    </td> 
		    <td height="25"><%=iCoverage%>%</td>     
		    <td height="25">Acceptable</td>
		</tr>
	  	<%
  	}
  	rs.close();
