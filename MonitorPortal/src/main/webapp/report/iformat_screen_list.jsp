...
RootURLNodePath="4"; 
LevelID="2";
String sql_aux=" and screenName<>'' and iFormatError=1 ";
workM.getConnection();
...
int iExecutedNode=0;
String sql_ExecutedNode="select count(*) from NodeList where iFormatError>0 and ProjectID='"+ProjectID+"'";
...
int iHardcodeNum=0;
sql2="select count(*) from NodeList where iFormatError=1 and ProjectID='"+ProjectID+"'";
...
<form method="POST" action="guilist.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    ...
    Total:<font color=blue><B><%=iTotalScreenNum%></B></font>, Coverage:<font color=blue><B>100%</B></font>
    <BR>
    iFormat error screen:<font color=red><B><%=iHardcodeNum%></B></font>
...
<%  
  ...
  while(RS.next())
  {
	  ...
	  {//·ÖÒ³
	  	...
	  	iFormatError=Integer.parseInt(RS.getString("iFormatError"));	  	
  ...
  <tr bgcolor="<%=bgcolor%>"> 
    <td height="25"> 
     ...
     <td height="25"><div align="left">
    	<a target='rightFrame' href='iformat_screen.jsp?ProjectID=<%=ProjectID%>&NodePath=<%=NodePath%>&screenName=<%=screenName%>'><%=screenName%></a>
    </div></td>
     <td height="25"><div align="left">
    	<%
    	if(iFormatError==1){
    	%>
    		<font color=red>Truncated</font>
    	<%
    	}else{
    	%>
    		<font color=green>Fine</font>
    	<%
    	}
    	%>
    </div></td>
