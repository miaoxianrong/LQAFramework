...
ResultSet rs=null; String sql=""; int iCoverage=0;
   ...
   if(false){
   String DropFileID=""; String KeyID=""; String AllDropFileID="";
   sql="select * from DropFile where ProjectID='00000265' and ReasonID>0 and mapped=1";
   ResultSet rs_mapped=workM.getURL(sql);   
   while(rs_mapped.next()){
   	   DropFileID=rs_mapped.getString("DropFileID");
   	   AllDropFileID=AllDropFileID+DropFileID+",";
   	   KeyID=rs_mapped.getString("KeyID");
   	   //out.print("DropFileID="+DropFileID+"<BR>");
   }
   StringTokenizer stArray1=new StringTokenizer(AllDropFileID,",");
   while(stArray1.hasMoreTokens()){
		DropFileID = stArray1.nextToken();
		sql="update DropFile set ReasonID=0 where DropFileID='"+DropFileID+"'";
		//out.println(sql+"<BR>");
		workM.updateSQL(sql);
   }
   rs_mapped.close();
   ...
   sql="select count(*) from DropFile where ReasonID=0 and NewFuzzy=1 and  ProjectID='"+ProjectID+"'"; 
   ...
   rs7.close();
   sql="select count(*) from DropFile where InScope=0 and ReasonID=0 and NewFuzzy=1 and ProjectID='"+ProjectID+"'";
   ResultSet rs8=workM.getURL(sql); 
   if(rs8.next()){
   	iInScope=rs8.getInt(1);
   }
   rs8.close();
   sql="select count(*) from DropFile where InScope=1 and ReasonID=0 and NewFuzzy=1 and ProjectID='"+ProjectID+"'";
...
<form method="POST" action="navigation.jsp" name="Maintain" >
  ...
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <td height="25">
    	<a target=_blank href="all.jsp?ProjectID=<%=ProjectID%>&ReasonID=-1&InScope=-1&mapped=-1&NewFuzzyAll=NewFuzzy&PreAnalysis=1"><font color='blue'><%=iAll%></font></a>
    </td>
    <td height="25"></td>
    <td height="25"></td>
  </tr> 
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <td height="25">   
    <a target=_blank href='all.jsp?ProjectID=<%=ProjectID%>&ReasonID=0&mapped=-1&NewFuzzyAll=NewFuzzy&InScope=0&PreAnalysis=1'><font color='red'><%=iInScope%></font></a>
    </td>
  ...
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <td height="25">
    <a target=_blank href='all.jsp?ProjectID=<%=ProjectID%>&ReasonID=0&mapped=-1&NewFuzzyAll=NewFuzzy&InScope=1&PreAnalysis=1'><font color='blue'><%=iOutScope%></font></a>
    </td>
  ...
  while(stArray.hasMoreTokens()){
  	...
  	rs.close();
  	sql="select count(*) from DropFile where ReasonID="+ReasonID+" and NewFuzzy=1 and ProjectID='"+ProjectID+"'"; 
  	rs=workM.getURL(sql); 
  	if(rs.next()){
  		iNumber=rs.getInt(1);
  		if(iAll>0){
	  	...
	  	<tr bgcolor="<%=bgcolor%>">     
		    ...
		    <td height="25">
		    <a target=_blank href='all.jsp?ProjectID=<%=ProjectID%>&ReasonID=<%=ReasonID%>&mapped=-1&InScope=-1&NewFuzzyAll=NewFuzzy&PreAnalysis=1'><font color='blue'><%=iNumber%></font></a>
		    </td> 
