...
<body bgcolor="#F8FDFF">
<%
String ReportID,ProjectID,ProductName,UserName,Depth2Coverage,Depth3Coverage,ScreenNumber,PlocNodeNumber,PlayNumber,Owner,plocNumber,fuzzyNumber,newStringNumber;
%>
<%
...
<form method="POST" action="navigation.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="17" align=center><span class="zi"><b>Navigation</b></span></td>
  </tr>
  <tr bgcolor="<%=bgcolor%>">     
...
<%  
  ...
  while(RS.next())
  {
	...
	*/
	...
	sql+=",Depth2Coverage="+iCoverage2;
	sql+=",Depth3Coverage="+iCoverage3;
   ...
   int iDepth3Coverage = 0;
   while(rs_get.next())
   {
   	ProjectID=rs_get.getString("ProjectID");
	ProductName=rs_get.getString("ProductName");
	UserName=rs_get.getString("UserName");
	Depth2Coverage=rs_get.getString("Depth2Coverage");	 
	Depth3Coverage=rs_get.getString("Depth3Coverage");	 
	iDepth3Coverage=Integer.parseInt(Depth3Coverage);
	...
	if(iDepth3Coverage<30){
		bgcolor="#F8CDFF";
	}else{
		if(iDepth3Coverage<50){
			bgcolor="yellow";
		}
		else{
			if(iDepth3Coverage<69){	
				bgcolor="yellow";	
			}else{
				bgcolor="green";	
			}
...
%>
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <td height="25"><%=ProductName%></td>
    <td height="25"></td>
    <td height="25"><%=Depth2Coverage%>%</td>
    <td height="25"><%=Depth3Coverage%>%</td>
    <td height="25"><%=ScreenNumber%></td>
    <td height="25"><%=PlocNodeNumber%></td>
    <td height="25"><%=PlayNumber%></td>
    <td height="25"><a href='ploc.jsp?ProjectID=<%=ProjectID%>'><%=plocNumber%></a></td>
    <td height="25"><a href='uistring.jsp?ProjectID=<%=ProjectID%>'><%=newStringNumber%></a></td>   
    ...
    <td height="25"><a href='guimap.jsp?ProjectID=<%=ProjectID%>'>GUIMap</a></td>
     <td height="25"><a href='develop.jsp?ProjectID=<%=ProjectID%>'>Develop</a></td>
  </tr>
  <%  	 
  	//}
  }  
