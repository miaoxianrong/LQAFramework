...
String get_special,DOMType,visible,branch,leaf,vector,uniqueObject,NodePath,TagOrURL;
...
RootURLNodePath="4"; 
LevelID="2";
String sql_aux=" and clicktimes>0";
workM.getConnection();
...
sql+=" order by clicktimes desc"; //touchtime
...
<form method="POST" action="status.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    ...
    <a href='status.jsp?ProjectID=<%=ProjectID%>'>All</a>
    <%
	...
	if(j==1){
		...
		middleNodePath=middleNodePath+"-"+getSeq;
     	...
     	%>
     	&nbsp;&nbsp;<a href='status.jsp?ProjectID=<%=ProjectID%>&node=<%=middleNodePath%>'><%=middleNodePath%></a>
     	<%
    }
    %>
    </b></span></td>
  ...
  <tr bgcolor="#F8FDFF"> 
    ...
    <td height="25"><div align="center">nodeHref</div></td>
...
%>
  ...
  <tr bgcolor="<%=bgcolor%>"> 
    ...
    <td height="25"><div align="left">
    	<%if(iLevelID==1){%>
    	<a href='status.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>'><%=NodePath%></a>
    	<%}%>
    </div></td>
     <td height="25"><div align="left">
    	<%if(iLevelID==2){%>
    ...
    <%if(bSpecial){
    	...
    	<a href='status.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>&debugging=quick'><font color='<%=runcolor%>'></font></a>
    <%}%>
    </div></td>
