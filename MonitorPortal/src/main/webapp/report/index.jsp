...
<body bgcolor="#F8FDFF">
<%
String ReportID,ProjectID,ProductName,Deepth2Coverage,Deepth3Coverage,ScreenNumber,PlayNumber,Owner,plocNumber,fuzzyNumber,newStringNumber;
%>
<%
String bgcolor="#F8FDFF";
String sql="select * from ProjectReport order by ProductName asc"; 
workM.getConnection();
...
<form method="POST" action="index.jsp" name="Maintain" >
  <input type=hidden name='ProjectID' value='<%=ProjectID%>'>
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="15" align=center><span class="zi"><b>Navigation</b></span></td>
  </tr>
  <tr bgcolor="<%=bgcolor%>">     
    <td height="25">ProjectID</td>
    <td height="25">ProductName</td>
    <td height="25">Coverage(2)</td>
    <td height="25">Coverage(3)</td>
    <td height="25">RecordScreen</td>
    <td height="25">MappingScreen</td>
    <td height="25">PLOCString</td>
    <td height="25">NewString</td>
    <td height="25">XPATH</td>
    <td height="25">Monitor</td>
    <td height="25">Filter</td>
    <td height="25">Log</td>
...
<%  
  ...
  while(RS.next())
  {
   ...
   while(stProjectID.hasMoreTokens()){
   	...
   	int iScreenNum=0;
	String sql_screen="select count(*) from NodeList where screenName<>'' and ProjectID='"+eachProjectID+"'";
	...
	String sql7="select newStringNumber from NodeList where ProjectID='"+eachProjectID+"'";
	ResultSet rs7=workM.getSQL(sql7);
	while(rs7.next()){
		iEachNodeNewNumber = Integer.parseInt(rs7.getString("newStringNumber"));
		iTotalNewNumber=iTotalNewNumber+iEachNodeFuzzyNumber;
	}
	rs7.close();
   ...
   sql="select * from ProjectReport order by ProductName asc"; 
...
%>
  <tr bgcolor="<%=bgcolor%>">     
    <td height="25"><%=ProjectID%></td>
    <td height="25"><%=ProductName%></td>
    <td height="25"><%=Deepth2Coverage%>%</td>
    <td height="25"><%=Deepth3Coverage%>%</td>
    <td height="25"><%=ScreenNumber%></td>
    <td height="25"><%=PlayNumber%></td>
    <td height="25"><%=plocNumber%></td>
    <td height="25"><%=newStringNumber%></td>
    <td height="25"><a href='../debug/xpath.jsp?ProjectID=<%=ProjectID%>'>XPATH</a></td>
    <td height="25"><a href='monitor.jsp?ProjectID=<%=ProjectID%>'>Monitor</a></td>
    <td height="25"><a href='filter.jsp?ProjectID=<%=ProjectID%>'>Filter</a></td>
    <td height="25"><a href='log.jsp?ProjectID=<%=ProjectID%>'>Log</a></td>
