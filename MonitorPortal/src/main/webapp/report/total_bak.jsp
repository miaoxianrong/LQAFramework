...
<form method="POST" action="total.jsp" name="Maintain" >
  ...
  <tr bgcolor="#F8FDFF"> 
    <td height="25" colspan="5">
			...
			<OPTION value="Fuzzy">Changed</OPTION>
			<OPTION value="NewFuzzy">New/Changed</OPTION>
	    </SELECT>&nbsp;    
	    Filter:
    	    <SELECT name=ReasonID size=1 class="tb">
	  		...
	  		EachNotFoundReason=rs_reason.getString("NotFoundReason");
    	    	%>
			<OPTION value="<%=EachReasonID%>"><%=EachNotFoundReason%></OPTION>
		<%
		}
		rs_reason.close();
		%>
...
%>
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <b>ID</b> = <%=UnitID%><BR>
    <b>File</b> = <a target=_blank href="total.jsp?FileNameListID=<%=FileNameListID%>&mapped=<%=mapped%>&ReasonID=<%=ReasonID%>&NewFuzzyAll=<%=NewFuzzyAll%>&ProjectID=<%=ProjectID%>"><%=FileName%></a>
    </div></td>
    ...
    if(mapped.compareTo("0")==0){    
    	...
    	%>
    		<a href="../develop/confirm_by_dev.jsp?InScope=<%=InScope%>&I18NFileID=<%=I18NFileID%>&ProjectID=<%=ProjectID%>"><IMG alt='confirm'  border=0 src="img/comment.gif"></a>
	<%
	}else{
	%>
		<%=CommentByDev%>-><a href="../develop/confirm_by_dev.jsp?InScope=<%=InScope%>&I18NFileID=<%=I18NFileID%>&ProjectID=<%=ProjectID%>"><font color=blue>update</font></a>
	<%    	
	}
    }else{
    	%>
