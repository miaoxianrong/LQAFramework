...
<body bgcolor="#F8FDFF">
<%
String UniqueStringListID,tagName,nodeId,screenName,nodeClass,nodeType,nodeTitle,childNum,nodeText,nodeHref,PageFilter;
String nodeName,nodeValue,ProjectID,LevelID,RootURLNodePath,seq,Active,onclick,onkeydown,Feature;
String get_special,DOMType,visible,branch,leaf,vector,uniqueObject,Hardcode,TagOrURL,newPloc,plocNumber;
...
if(getNodePath!=null){
	...
	if(searchNodePath.compareTo("")==0){
		...
		sql_aux=sql_aux+" and Hardcode like '"+searchNodePath+"-%'";
		String debugging=request.getParameter("debugging");
		if(debugging!=null){
			if(debugging.compareTo("quick")==0){
				String sql1="insert into NodeTest set Hardcode='"+getNodePath+"',ProjectID='"+ProjectID+"',Active=1";
...
if(bSearch){
	iOnePage=1000;
	sql="select count(*)  from UniqueStringList";
	if(SearchContent.compareTo("")==0){
		sql+=" where UniqueStringListID>0 ";
	}
	else{
...
if(bSearch){
	...
	sql="select * from UniqueStringList";	
	sql+=" where ProjectID='"+ProjectID+"' "+ sql_aux;
}
sql+=" order by Hardcode asc"; //touchtime
...
<form method="POST" action="hardcode_string.jsp" name="Maintain" >
  <input type=hidden name='ProjectID' value='<%=ProjectID%>'>
  <tr bgcolor="#C3DBE8"> 
    <td height="25" colspan="2" align=left><div align="left">
    ...
    Found hard code strings:
    <a target='rightFrame' href='hardcode_screen_string.jsp?ProjectID=<%=ProjectID%>&Hardcode=000&screenName=000.png'>
  ...
  <tr bgcolor="#F8FDFF"> 
    ...
    <td height="25"><div align="center">Hard code string</div></td>
  </tr>
...
<%  
  ...
  while(RS.next())
  {
	  ...
	  {//·ÖÒ³
		...
		%>
		  ...
		  <tr bgcolor="<%=bgcolor%>"> 
		    <td height="25"> 
		      <div align="center"><INPUT name=now_dn type=checkbox value="<%=UniqueStringListID%>"></div>
		    </td>
		     <td height="25"><div align="left">
		    	<a target='rightFrame' href="hardcode_screen.jsp?ProjectID=<%=ProjectID%>&UniqueStringListID=<%=UniqueStringListID%>"><%=Hardcode%></a>
		    </div></td>
  ...
  %>
   <tr bgcolor="#F8FDFF"> 
    <td height="25" colspan="2"> 
      <div align="left">Total nodes: <%=iTotalRecords%> </div></td>
  </tr>
