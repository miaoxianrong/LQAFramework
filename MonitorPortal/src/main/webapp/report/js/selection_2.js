...
function CompareShu(theValue){
	  alert(thisForm.TotalShuliang.value);
	//iTotalShuliang=parseInt(thisForm.TotalShuliang.value)
	iTotalShuliang=parseInt("99")
	alert(iTotalShuliang);
	if(theValue<iTotalShuliang){
		alert(">");                   
		return false;   
	}
	if(theValue>iTotalShuliang){
		...
		for (i = 0; i <thisForm.now_dn.length; i++)
		{
			...
			if (thisForm.now_dn[i].checked==true)
			{
				...
				if(checkvalue(thisForm.i_jiage[i].value)){
					iShuliangHeji=iShuliangHeji+parseInt(thisForm.i_jiage[i].value);
