...
<form method="POST" action="navigation.jsp" name="Maintain" >
  ...
  <tr bgcolor="<%=bgcolor%>">     
    ...
    <td height="25">    
    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=0&NewFuzzyAll=NewFuzzy&mapped=0'><font color='red'><%=iUnknown%></font></a>: All main UI 
    <BR>
    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=0&NewFuzzyAll=NewFuzzy&mapped=0&InScope=0'><font color='red'><%=iInScope%></font></a>: In scope
    <BR>
    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=0&NewFuzzyAll=NewFuzzy&mapped=0&InScope=1'><font color='red'><%=iOutScope%></font></a>: Not in scope
    </td>
  ...
  while(stArray.hasMoreTokens()){
  	...
  	if(rs.next()){
	  	...
	  	<tr bgcolor="<%=bgcolor%>">     
		    ...
		    <td height="25">
		    <a target=_blank href='total.jsp?ProjectID=<%=ProjectID%>&ReasonID=<%=ReasonID%>&NewFuzzyAll=NewFuzzy&mapped=0'><font color='blue'><%=iNumber%></font></a>
		    </td> 
