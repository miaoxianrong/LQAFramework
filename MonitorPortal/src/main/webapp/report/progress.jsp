...
<title>play(mapping,i18n) progress</title>
...
RootURLNodePath="4"; 
LevelID="2";
String sql_aux=" and clickable=1 and special=1 and clickfail=0 and validNode=1 and LevelID>0";
workM.getConnection();
...
int iTotalSpecial=0;
String sql3="select count(*) from NodeList where clickable=1 and special=1 and clickfail=0 and validNode=1 and ProjectID='"+ProjectID+"'";
...
//sql+=" order by playonce desc"; //touchtime
sql+=" order by nodepath asc";
...
<form method="POST" action="progress.jsp" name="Maintain" >
  ...
  <tr bgcolor="#C3DBE8"> 
    ...
    <a href='progress.jsp?ProjectID=<%=ProjectID%>'>All</a>
    <%
	...
	if(j==1){
		...
		middleNodePath=middleNodePath+"-"+getSeq;
     	...
     	%>
     	&nbsp;&nbsp;<a href='progress.jsp?ProjectID=<%=ProjectID%>&node=<%=middleNodePath%>'><%=middleNodePath%></a>
     	<%
    }
    %>
    </b></span></td>
...
%>
  ...
  <tr bgcolor="#AEF2F4"> 
    ...
    <td height="25" bgcolor="<%=bgcolor%>"><div align="left">
    	<%if(iLevelID==1){%>
    	<a href='progress.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>'><%=NodePath%></a>
    	<%}%>
    </div></td>
     <td height="25" bgcolor="<%=bgcolor%>"><div align="left">
    	<%if(iLevelID==2){%>
    ...
    <%if(bSpecial){
    	...
    	<a href='progress.jsp?ProjectID=<%=ProjectID%>&node=<%=NodePath%>&debugging=quick'><font color='<%=runcolor%>'></font></a>
    <%}%>
    </div></td>
    <td height="25"><div align="center">
    <a target='rightFrame' href='../screen/<%=Feature%><%=screenName%>'><%=screenName%></a></div></td>
